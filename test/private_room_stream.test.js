...
describe('private_room_stream 行为锚定（#8 修复点）', () => {
  ...
  // 16. rewindToLastUser 直测:截到最近 user(含),删其后 assistant
  it('ChatHistory.rewindToLastUser:截到最近 user(含),删其后 assistant', () => {
    history.addMessage(roomId, 'user', 'u1');
    history.addMessage(roomId, 'assistant', 'a1');
    history.addMessage(roomId, 'assistant', 'a2');
    const changed = history.rewindToLastUser(roomId);
    assert.equal(changed, true, '应实际截断');
    const recs = fs.readFileSync(path.join(root, roomId, 'history.jsonl'), 'utf-8').trim().split('\n').map(l => JSON.parse(l));
    assert.equal(recs.length, 1, '只剩 user 一条');
    assert.equal(recs[0].role, 'user');
  });

  it('ChatHistory.rewindToLastUser:末条已是 user 不动;无 user 不动', () => {
    history.addMessage(roomId, 'user', 'u1');
    assert.equal(history.rewindToLastUser(roomId), false, '末条已是 user → no-op');
    let recs = fs.readFileSync(path.join(root, roomId, 'history.jsonl'), 'utf-8').trim().split('\n');
    assert.equal(recs.length, 1, '仍只一条');

    const root2 = fs.mkdtempSync(path.join(os.tmpdir(), 'elf-prs2-'));
    const h2 = new ChatHistory(root2, root2, { roomMode: true, roomsDir: root2 });
    h2.addMessage(roomId, 'assistant', 'a1');
    assert.equal(h2.rewindToLastUser(roomId), false, '无 user → no-op(不动)');
    recs = fs.readFileSync(path.join(root2, roomId, 'history.jsonl'), 'utf-8').trim().split('\n');
    assert.equal(recs.length, 1, '无 user 时 assistant 保留不误删');
    fs.rmSync(root2, { recursive: true, force: true });
  });

  // 17. aborted 把 mid-round 已 flush 落盘的 assistant 气泡从 history.jsonl 截掉,回到最近 user(含)
  //     复盘 elf-018:outline→render 切换时 outline 气泡已 flush 落盘,'aborted' 只清内存 partial 救不回;
  //     须把 history.jsonl 截到最近 user,与 agent 侧 rewindToLastUser(context.json)/runtime 还原一致。
  it('aborted 截掉 mid-round 已 flush 的 assistant 气泡,history 回到本轮 user', () => {
    beginTurn('玩家本轮指令', null);
    // outline loop:status 切 outline → 工具调用 → 结果
    handlePrivateAgentEvent('status', { _roomId: roomId, state: 'thinking', loop: 'outline' }, history);
    handlePrivateAgentEvent('tool_call', { _roomId: roomId, loop: 'outline', tool_calls: [{ id: 'tc1', name: 'WriteOutline', args: {} }] }, history);
    handlePrivateAgentEvent('tool_result', { _roomId: roomId, id: 'tc1', status: 'success', message: '大纲已落' }, history);
    // outline→render 切换:status{render} 触发 flushBubble,把 outline 气泡落盘
    handlePrivateAgentEvent('status', { _roomId: roomId, state: 'thinking', loop: 'render' }, history);
    const before = fs.readFileSync(path.join(root, roomId, 'history.jsonl'), 'utf-8').trim().split('\n').map(l => JSON.parse(l));
    assert.equal(before.filter(r => r.role === 'assistant').length, 1, 'abort 前已 flush 出 outline 气泡');
    // render 首 token 后用户中止
    handlePrivateAgentEvent('token', { _roomId: roomId, loop: 'render', content: '剧情' }, history);
    handlePrivateAgentEvent('aborted', { _roomId: roomId }, history);
    const after = fs.readFileSync(path.join(root, roomId, 'history.jsonl'), 'utf-8').trim().split('\n').map(l => JSON.parse(l));
    assert.equal(after.length, 1, 'abort 后只剩本轮 user 一条');
    assert.equal(after[0].role, 'user', '末条即本轮 user');
    assert.equal(after.filter(r => r.role === 'assistant').length, 0, 'mid-round 已 flush 的 outline 气泡被截掉,不残留');
  });
});