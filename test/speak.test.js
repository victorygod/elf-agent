...
import { Speak } from '../shared/agent/tools/Speak.js';

/** 起一个 mock room_bus server，捕获 member-said 请求 */
function startMockRoomBus() {
  const received = [];
  let status = 200;
  const srv = http.createServer((req, res) => {
    if (req.url === '/member-said' && req.method === 'POST') {
      let body = '';
...
describe('Speak 工具', () => {
  ...
  it('群聊模式 execute → fetch member-said,带 member/content', async () => {
    ...
    assert.equal(mock.received[0].member, 'elf-001');
