...
export default function RoomConfigDrawer({ roomId, onClose }) {
  ...
  const handleRemove = async (agentId) => {
    if (!confirm(`移除成员 ${agentId}?`)) return;
    setBusy(true);
    try {
  ...
  const handleDelete = async () => {
    if (!confirm(`解散群「${room.name}」?此操作不可恢复,所有成员记忆将删除。`)) return;
    setBusy(true);
    try {
  ...
  const handleClearAll = async () => {
    if (!confirm('清空本群的聊天记录 + 所有成员在本群的记忆?不影响各自私聊记忆。')) return;
    setBusy(true);
  ...
  return (
    <div className={styles.drawer}>
              ...
              <div key={id} className={styles.memberItem}>
                <div className={styles.memberLeft}>
                  <Avatar
                    ...
                    bgColor="#4a90d9"
                    fallback={(a?.name || id).charAt(0).toUpperCase()}
                    size={28}
                  />
                  <span className={styles.memberName}>{a?.name || id}</span>
                  <span className={`${styles.statusDot} ${isRunning ? styles.online : styles.offline}`} title={isRunning ? '在线' : '离线'} />
                </div>
                <button className={styles.removeBtn} onClick={() => handleRemove(id)} disabled={busy}>移除</button>
              </div>
            );
          })}
        </div>
      ...
      <div className={styles.section}>
        ...
        <button className={styles.dangerBtn} onClick={handleClearAll} disabled={busy}>清空聊天记录与成员记忆</button>
      </div>

      <div className={styles.section}>
        <button className={styles.deleteBtn} onClick={handleDelete} disabled={busy}>解散群</button>
      </div>
