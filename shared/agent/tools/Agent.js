 ...
 *
 * 引擎部件获取：execute 经 ctx.agent 拿主 agent 引用（ToolRegistry.execute 透传 ctx）。
 */
...
export const Agent = {
   ...
   * @param {object} ctx - 工具上下文 { agent } 主 agent 实例（ToolRegistry.execute 透传）
   */
  execute: async (args, signal, ctx) => {
    ...
    const parentTools = parentAgent.toolRegistry.getAll();
    ...
    const ToolRegistryCtor = parentAgent.toolRegistry.constructor;
    const subRegistry = new ToolRegistryCtor();
    ...
    let finalText = '';
    try {
      for await (const _event of subAgent.receive(prompt)) {
        // 吞掉子 agent 的流式事件（不转发前端），drain 到 loop 结束
      }
