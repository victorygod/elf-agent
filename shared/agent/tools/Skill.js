 ...
 * 并记录到 agent._invokedSkills 供 compact 恢复（对齐 $O6）。
 */
...
export const Skill = {
  ...
  execute: async (args, signal, ctx) => {
    ...
    if (!agent || !agent._skillRegistry) {
      return `Error: skills not enabled on this agent`;
    }

    const skill = agent._skillRegistry.get(name);
    if (!skill) {
    ...
    if (Array.isArray(agent._invokedSkills)) {
      agent._invokedSkills.push({ name, path: skill.skillRoot, contents: [body] });
    }
