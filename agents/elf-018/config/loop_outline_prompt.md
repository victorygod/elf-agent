...
- **写到需判定处，先把至此的剧情写进大纲内容，再调 `Roll` 取结果，按 `（用途 / d20=骰值 vs DC / 成功|失败）` 续写其后；严禁未写出前情就先 Roll。** 多次 roll 各自穿插在对应交锋处。
...
- **战斗轮**：据策略一战到底——判定每次 roll 一行（用途/技能/DC/modifier/骰值/结果）；结算多敌人按威胁顺序，击败按 exp 规则（见 system），达阈值升级连带技能点/HP·MP 上限。
