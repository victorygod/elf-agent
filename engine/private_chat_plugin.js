 ...
 *   action:'buffer' + flushLoop。差异只在 flushNow = shouldFlush() = !_replying——
 *   一空闲就 flush，语义等价于前端"发一条屏蔽输入，等回复完再发"。
 *
 * sync（align/advance）：accept 内做 align；advance 由基类 receive 统一调（私聊/群聊一致）。
 *   addUserMessage 不在此做（flushLoop 的 mergeForReason→addUserMessage 统一加，防双份）。
 *
...
export class PrivateChatPlugin extends ScenePlugin {
   ...
   *   不做 addUserMessage（flushLoop 统一加）；advance 由 receive 统一调。
   */
