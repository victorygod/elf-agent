 ...
 *   - mergeForReason / postReason / _flushPending / replying / shouldContinue（flush 循环原语）
 *   - preReceive 默认骨架（accept → replying?pending:buffer → flushNow=shouldFlush()）
 *   - reasoning gate 默认 no-op
 *   注：flush 循环本身已内联于 default_agent.receive（每轮 merge→addUser→reasoning→postReason），
 *       ScenePlugin 只留 buffer 机器原语供循环驱动。
 *
 * 子类 override 差异点：
...
export class ScenePlugin {
  ...
  /** 标记当下是否在 reasoning（flush 循环每轮 enter/exit）。供 receive 内联循环调。 */
  replying(v) { this._replying = v; }

  /** flush 循环是否应继续下一轮：buffer 非空且 shouldFlush。供 receive 内联循环判。 */
  shouldContinue() { return this._buffer.length > 0 && this.shouldFlush(); }
  ...
  /** flush 循环每轮取合并文本，清 buffer + mention 标记。first-wins 合并语义。 */
  mergeForReason() {
  ...
  /** flush 循环每轮 reasoning 后：drain pending，返回是否再 flush 一轮。OR 合并语义。 */
  ...
  // flush 循环内联于 default_agent.receive 的 chat 分支（merge→addUser→reasoning→postReason，while shouldFlush）。
  //   本类只留 buffer 机器原语（mergeForReason/postReason/shouldFlush/shouldContinue/replying）供循环驱动。
