...
## 15. 实现状态（A/B/C 阶段落地记录）

> 本节记录设计落地后的实际状态，与上述设计快照（§7 等）的差异以本节为准。

### A 阶段（gateway 内核）：✅ 完成
- `RoomBroadcaster` / `RoomHistory` / `RoomRegistry` / `RoomConfig` 纯逻辑内核
- `allocPort` 动态端口分配

### B 阶段（真实副本 spawn + /observe）：✅ 完成
- 真实 `spawn` 副本进程（`--mode room`），detached 常驻
- `/observe` 端点 + 独立队列（`pendingObserve`，出队逐条还原投递）
- `ensureReplicasAlive` 保活巡检 + run.json re-discover
- gateway 重启自动恢复副本

### C 阶段（路由统一 + 消息下发收敛）：✅ 完成

**统一订阅模型**：`processRoomMessage` → `RoomBroadcaster.notifyAll()` 统一通知 SSE（前端）+ POST /observe（agent），调用方（`/say` 路由）无感知。

**统一发言入口 `/say`**（替换原 `/send` + `/member-said`）：
```
POST /rooms/:rid/say
  Headers: X-Speaker-Id: <user | agentId>
  Body: { content }
```
身份走 header，`user` → 用户发言，成员 agentId → agent 发言。原 `/send` `/member-said` 已删（零兼容）。

**ID/Name 分离**（uid 作唯一标识、name 作显示首选）：
- 落盘（group-history.jsonl）：`speaker`/`speakerUid` = uid，content `@` = uid
- 发送：gateway 发给前端/agent 前统一 `rewriteMentions` 把 `@uid` 改成 `@name`、speaker uid→name
- agent `/observe` 的 `from` = uid（自消息过滤靠 uid，agent 改名不影响过滤），`_parse` 映射 uid→name 拼前缀

详见 `docs/room-gateway-agent-api.md`。

### 修复记录

- **pendingObserve 出队 payload 形状 bug**（elf-001 停死根因）：合并存储 `{froms,contents,mentions:Set}` 出队时未还原成 `{from,content,mentions:[],role:'chat',seq}`，被 RoomAgent.receive 误判为"非chat消息"走私聊路径触发空 reasoning。修复：出队逐条还原投递。
- **seq 类型 bug**：`/member-said` 曾把 `rec.id`（字符串 `rmsg_xxx`）当 seq 传给 broadcastObserve，导致 `_processedSeqs` 去重失效、消息重复。修复：改传 `rec.seq`（数字）+ `_advanceCursor` 防御性 `Number()`。
- **room 副本状态显示**：`RoomConfigDrawer` 曾用全局 agentStore 的私聊进程状态，改用 roomStore 的副本状态。
- **`_replying` 期间消息丢弃**：agent reasoning 中收到消息曾直接 return 丢弃，改为入 `_pendingBuffer` 队列，reasoning 结束后 flush。
