...
export class AggregatedBroadcaster {
  ...
  /** 前端 POST /subscribe 调:建立一条聚合 SSE。 */
  attach(res) {
    res.writeHead(200, {
  ...
  refresh() {
    const current = this._currentRooms();
  ...
  /** 当前应订阅的房:所有 running agent 的 chat-<id> + 所有群聊房。 */
  _currentRooms() {
    const rooms = [];
    try {
      const agents = this._pm.listAgents?.() || [];
      for (const a of agents) {
        if (a.status === 'running') rooms.push({ roomId: `chat-${a.agentId}`, type: 'chat' });
      }
    } catch (e) { /* ignore */ }
    try {
